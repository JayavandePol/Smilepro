...
{{-- Requirement 2.1: Tailwind responsive layout ensures the table + controls scale across breakpoints. --}}
@section('content')
<div class="space-y-8">
    ...
    <div class="bg-white dark:bg-gray-800 rounded-2xl shadow-xl border border-gray-200 dark:border-gray-700 overflow-hidden">
        <div class="px-6 py-4 flex flex-wrap gap-4 items-center justify-between border-b border-gray-100 dark:border-gray-700">
            ...
            <form method="GET" action="{{ route('dashboard.users') }}" class="flex flex-wrap items-center gap-3">
                ...
                <select name="role" class="rounded-full border border-gray-300 bg-white px-4 py-2 text-sm text-gray-700 shadow-sm focus:border-blue-500 focus:outline-none focus:ring dark:border-gray-700 dark:bg-gray-900 dark:text-gray-100">
                    ...
                    @foreach(($roleOptions ?? collect()) as $role)
                        <option value="{{ $role }}" @selected(($activeRole ?? '') === $role)>{{ $role }}</option>
                    @endforeach
                </select>
                ...
                @if(!empty($activeRole))
                    <a href="{{ route('dashboard.users') }}" class="rounded-full border border-gray-300 px-4 py-2 text-sm text-gray-600 hover:bg-gray-50 dark:border-gray-600 dark:text-gray-300 dark:hover:bg-gray-800">Reset</a>
                @endif
            </form>
        </div>
            ...
            <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-700">
                <thead class="bg-gray-50 dark:bg-gray-900">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-right text-xs font-semibold uppercase tracking-wide text-gray-500 dark:text-gray-400">Acties</th>
                    </tr>
                </thead>
                    ...
                    <tr class="hover:bg-gray-50 dark:hover:bg-gray-900 transition">
                        ...
                        <td class="px-6 py-4 text-right">
                            <div class="inline-flex gap-2">
                                <button class="rounded-full border border-gray-300 dark:border-gray-600 px-4 py-1 text-sm text-gray-700 dark:text-gray-200 hover:bg-gray-100 dark:hover:bg-gray-700">
                                    Bewerken
                                </button>
                                <button class="rounded-full border border-red-300 text-red-500 px-4 py-1 text-sm hover:bg-red-50 dark:border-red-700 dark:text-red-300 dark:hover:bg-red-900/40">
                                    Verwijderen
                                </button>
                            </div>
                        </td>
